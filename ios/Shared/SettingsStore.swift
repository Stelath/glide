...
final class SettingsStore {
    ...
    var llmEnabled: Bool { didSet { persistBool(llmEnabled, key: "llm_enabled") } }
    ...
    func reloadFromDisk() {
        ...
        sttProvider = store.string(forKey: "stt_provider") ?? ProviderInfo.openAI.id
        sttModel = store.string(forKey: "stt_model") ?? "whisper-1"
        llmEnabled = store.bool(forKey: "llm_enabled")
        llmProvider = store.string(forKey: "llm_provider") ?? ProviderInfo.openAI.id
    ...
    func sttModels(for provider: String) -> [String] {
        ...
        return sttModelsByProvider[normalized] ?? (normalized == ProviderInfo.groq.id ? [] : ["whisper-1"])
    }
