...
/// Whether a provider's API key has been verified via a successful /models fetch.
...
/// Whether at least one provider has been verified.
pub fn any_provider_verified() -> bool {
    provider_verified(Provider::OpenAi) || provider_verified(Provider::Groq)
}

/// Recommended STT model based on verified providers. Prefers Groq.
...
/// Recommended LLM model based on verified providers. Prefers Groq.
...
/// Fix models pointing at unverified providers. Does not auto-enable a disabled LLM.
...
/// Keep only models from verified providers. Returns all if none verified yet.
...
mod tests {
    ...
    // Guard for tests that mutate PROVIDER_VERIFIED (process-global static).
    ...
    fn test_apply_smart_defaults_initial_does_not_re_enable_llm_after_flag_set() {
        ...
        // Second call must NOT re-enable it
        apply_smart_defaults_initial(&mut config);
        assert!(config.dictation.llm.is_none());
